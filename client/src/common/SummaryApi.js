...
const SummaryApi = {
  ...
  signin: async (email, password) =>
    apiRequest("/api/auth/signin", "POST", { email, password }, true),
  ...
  upgradeSemester: async (currentSemester, upgradeSemester, studentIds) =>
    apiRequest("/api/students/upgrade-semester", "POST", {
      currentSemester,
      upgradeSemester,
      studentIds, // Include selected student IDs for upgrading
    }),
  ...
  updateStudentResults: async (studentId, semester, marksData) =>
    apiRequest(`/api/students/${studentId}/semester/${semester}`, "PUT", {
      results: marksData.results,
    }),
  updateMarksForSemester: async (data) =>
    apiRequest("/api/students/update-marks", "POST", data),
  getTopStudentForSemester: async (semester) =>
    apiRequest(`/api/students/topStudentForSemester/${semester}`, "GET"),
  calculateCGPA: async (studentId, semester) =>
    apiRequest(`/api/students/calculate-cgpa/${studentId}`, "GET"),
  calculateGPA: async (studentId) =>
    apiRequest(`/api/students/calculate-gpa/${studentId}`, "GET"),
  ...
  getSemesterWiseCount: async () =>
    apiRequest("/api/students/admin/semester-wise-count", "GET"),
  getSemesterWisePerformance: async () =>
    apiRequest("/api/students/admin/semester-wise-performance", "GET"),

  // Scholarship Management
  addScholarship: async (scholarshipData) =>
    apiRequest("/api/rewards/scholarships", "POST", scholarshipData),
